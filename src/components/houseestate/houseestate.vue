			...
			<div class="container">
				...
				<div class="leftContent">
					<div class="orderFilter">
						...
						<div class="resultDes">
							<h2 class="total">共找到<span style="color: red;"> {{buildlistcount.count}} </span>套{{this.city}}二手房</h2>
					...
					<div class="item">
						<ul>
							<li v-for="item in buildlist">
								...
								<div class="direciton">
									<div class="introduce intrdex " @click="toSkip(item)" >{{item.buildName}}</div>
									<div class="introduce"><img src="../../imgs/buyhouse/dingwei.png" />
										...
										<span class="prices">{{item.avgSalePrice}}<span class="wan">元/平米</span></span>
                                        <span class="fr count ">{{item.saleCount}}<span style="font-size:18px;font-weight:bold;">套</span></span>
                                        </div>
									<div class="introduce"><img src="../../imgs/buyhouse/house.png" />
                                    <span class="word">在售{{item.saleCount}}套/在租{{item.rentCount}}套</span>
                                     <span class="cankaojia">小区参考均价</span>
                                    </div>
								</div>
							</li>
						</ul>
...
<script>
	...
	export default {
		...
		created() {	
			...
			//获取搜索小区总数量
			this.$http.post(this.$url.URL.BUILDLIST_COUNT, {  
				         scity: this.params.scity,
				         pageNo :1
			...
			//请求搜索条件
			this.$http.get(this.$url.URL.AREA_DISTRICTS + this.params.scity) //区域
			.then((response)=>{
				this.listone = response.data.data
			})
			this.$http.get(this.$url.URL.DICTIONARY_DICTYPE + "SELL_UNIT_PRICE" + "/" + this.params.scity) //均价
			.then((response)=>{
				this.listtwo = response.data.data
			}) 	
			this.$http.get(this.$url.URL.DICTIONARY_DICTYPE + "HOUSE_AGE") //楼龄
			.then((response)=>{
		...
		methods: {
			...
			requestServerData(params) {  //请求过滤搜索条件数据
				this.$http.post(this.$url.URL.BUILDLIST , params)
				.then((response)=>{
					this.buildlist = response.data.data
				})
			},
			requestCountData(params) {   //请求小区房源数量
				this.$http.post(this.$url.URL.BUILDLIST_COUNT , params)
				.then((response)=>{
					this.buildlistcount = response.data.data
				})
			},
